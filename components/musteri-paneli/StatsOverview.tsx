...
const CustomTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-slate-800 text-white text-xs font-bold px-2.5 py-1 rounded shadow-md">
        {payload[0].value}
      </div>
    );
  }
...
export function StatsOverview() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5">
      <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm flex flex-col gap-3.5">
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm flex flex-col gap-3.5">
        ...
        <div className="flex items-end justify-between">
          ...
          <span className="text-xs font-semibold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded flex items-center gap-1">
            +4.1% <TrendingUp size={13} />
          </span>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-lg p-5 cursor-pointer hover:shadow-md transition-all shadow-sm flex flex-col justify-between relative overflow-hidden">
        <div className="absolute top-0 right-0 w-28 h-28 bg-white opacity-10 rounded-full -mr-10 -mt-10 blur-2xl pointer-events-none" />
        ...
        <div className="flex justify-between items-center relative z-10">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-emerald-50 tracking-wider">KREDİ BAKİYE</span>
          </div>
        ...
        <div className="relative z-10 mt-3">
          ...
          <div className="text-xs font-medium text-emerald-100/80">≈ 29.150 ₺</div>
        </div>
      </div>
    </div>
  );
